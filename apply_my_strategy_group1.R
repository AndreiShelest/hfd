...
for (selected_quarter in c("2022_Q1", "2022_Q3", "2022_Q4", 
  ...
  # lets calculate EMA10 and EMA180 for NQ
  data.group1$NQ_EMA10 <- EMA(na.locf(data.group1$NQ), 10)
  data.group1$NQ_EMA180 <- EMA(na.locf(data.group1$NQ), 180)
  
  # and for SP we use EMA15 and EMA180
  data.group1$SP_EMA15 <- EMA(na.locf(data.group1$SP), 15)
  ...
  data.group1$NQ_EMA180[is.na(data.group1$NQ)] <- NA
  data.group1$SP_EMA15[is.na(data.group1$SP)] <- NA
  data.group1$SP_EMA180[is.na(data.group1$SP)] <- NA
  
  
  # lets calculate the position for the MOMENTUM strategy
  # With two moving averages - crossover of fast and slow EMA
  ...
  data.group1$positionNQ.mom <- ifelse(lag.xts(data.group1$NQ_EMA10) >
                                         lag.xts(data.group1$NQ_EMA180),
                                       1, -1)
  
  data.group1$positionSP.mom <- ifelse(lag.xts(data.group1$SP_EMA15) >
                                         lag.xts(data.group1$SP_EMA180),
                                       1, -1)
